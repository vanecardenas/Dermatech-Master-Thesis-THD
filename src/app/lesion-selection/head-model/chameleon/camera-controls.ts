...
var mouseProjectionOnBall = (() => {
  ...
  return (
    ...
    projLocal.set(
      (event.pageX - canvasBox.width * 0.5 - canvasBox.left) /
        (canvasBox.width * 0.5),
      (canvasBox.height * 0.5 + canvasBox.top - event.pageY) /
...
export class CameraControlsBase {
  ...
  _eye: Vector3 = new THREE.Vector3();
  target: Vector3 = new THREE.Vector3();
  _rotateStart: Vector3 = new THREE.Vector3();
  _rotateEnd: Vector3 = new THREE.Vector3();
  _zoomStart: number = 0;
  _zoomEnd: number = 0;
  _panStart: Vector2 = new THREE.Vector2();
  _panEnd: Vector2 = new THREE.Vector2();
  ...
  constructor(public camera: Camera, public canvasBox: Box) {}
  ...
  panCamera = (() => {
    ...
    return () => {
      ...
      if (mouseChange.lengthSq()) {
        ...
        pan
          .crossVectors(this._eye, this.camera.up)
          .setLength(mouseChange.x)
          .add(cameraUp.copy(this.camera.up).setLength(mouseChange.y));
...
export class PerspectiveCameraControls extends CameraControlsBase {
  ...
  constructor(
    public override camera: PerspectiveCamera,
    public override canvasBox: Box
  ) {
