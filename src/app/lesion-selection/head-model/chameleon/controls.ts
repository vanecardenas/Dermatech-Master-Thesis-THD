...
import { Brush, Pencil } from './brushes';
import {
  OrthographicCameraControls,
  PerspectiveCameraControls,
} from './camera-controls';
import { Box } from './chameleon';
import { mousePositionInCanvas } from './common';
...
export class Controls {
  ...
  public updateCanvasBox() {
    ...
    this.canvasBox.left =
      canvasRect.left + window.pageXOffset - docElement.clientLeft;
    this.canvasBox.top =
      canvasRect.top + window.pageYOffset - docElement.clientTop;
    this.canvasBox.width = canvasRect.width;
    this.canvasBox.height = canvasRect.height;
  }

  private _headLight: PointLight = new THREE.PointLight(0xffffff, 0.25);
  ...
  update() {
    ...
    if (ctx) ctx.drawImage(this._renderer.domElement, 0, 0);
  }
  ...
  private _mousewheel = (event: WheelEvent) => {
    ...
    if (this._state === ControlsState.Draw || !this.perspectiveView) {
      return;
    }
  ...
  private static _computeBoundingBallRadius(geometry: BufferGeometry): number {
    var radius = 0;
    var origin: Vector3 = new THREE.Vector3(0, 0, 0);
    for (
      var i = 0;
      i < geometry.getAttribute('position').array.length;
      i += 1
    ) {
      ...
      radius = Math.max(radius, vector3Position.distanceTo(origin));
    }
    return radius;
  }

  private _initializeCamera() {
    this._boundingBallRadius = Controls._computeBoundingBallRadius(
      this.geometry
    );

    var fov = 60;
    var z =
      (2 * this._boundingBallRadius) / Math.tan((fov / 2 / 180) * Math.PI);
    ...
    this._orthographicCameraControls = new OrthographicCameraControls(
    ...
    this._perspectiveCameraControls = new PerspectiveCameraControls(
  ...
  //   zip.file(
  //     'texture.png',
  //     textureDataUrl.substr(textureDataUrl.indexOf(',') + 1),
  //     { base64: true }
  //   );
  ...
  constructor(geometry: BufferGeometry, canvas?: HTMLCanvasElement) {
    ...
    this.canvas.addEventListener('wheel', this._mousewheel, false);
    // this.canvas.addEventListener('scroll', this._mousewheel, false); // firefox
